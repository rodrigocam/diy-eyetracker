...
class Calibration:
    ...
    def write(self, out):
        resx = curve_fit(model, list(self.xes.values()), list(map(lambda p: p[0], self.xes.values())), method="lm")[0]
        resy = curve_fit(model, list(self.xes.values()), list(map(lambda p: p[1], self.xes.values())), method="lm")[0]
