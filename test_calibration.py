...
class Calibration:
    ...
    def write(self, out):
        ...
        self.reg.fit(list(self.xes.values()), list(self.xes.keys()))
...
while True:
    ...
    result = detector.detect(eye_img)
    xe = result["ellipse"]
    prediction = calibration.reg.predict([xe["center"]])
    _, scene_img = webcam.read()
    scene_img = cv2.resize(scene_img, (800, 600))
    #print(prediction)
    cv2.circle(scene_img, (int(prediction[0][0]), int(prediction[0][1])), 30, (255, 0, 0), 2)
